#!/bin/bash
# common.sh — shared functions for Layer 4.6 scripts

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SCFUTURE_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"
IP_FILE="$SCRIPT_DIR/.ips"

NETWORK_NAME="l46-net"
SSH_KEY_NAME="l46-key"
LOCATION="nbg1"
SERVER_TYPE="cx23"
IMAGE="ubuntu-24.04"
...
save_ips() {
    ...
    cat > "$IP_FILE" << EOF
COORD_PUB_IP=$(hcloud server ip l46-coordinator)
COORD_PRIV_IP=$(hcloud server describe l46-coordinator -o json | jq -r '.private_net[0].ip')
FLEET1_PUB_IP=$(hcloud server ip l46-fleet-1)
FLEET1_PRIV_IP=$(hcloud server describe l46-fleet-1 -o json | jq -r '.private_net[0].ip')
FLEET2_PUB_IP=$(hcloud server ip l46-fleet-2)
FLEET2_PRIV_IP=$(hcloud server describe l46-fleet-2 -o json | jq -r '.private_net[0].ip')
FLEET3_PUB_IP=$(hcloud server ip l46-fleet-3)
FLEET3_PRIV_IP=$(hcloud server describe l46-fleet-3 -o json | jq -r '.private_net[0].ip')
EOF
...
EOF
    ...
    sleep 8  # give reconciliation extra time to finish all operations
}
