...
# Configure coordinator with DATABASE_URL (with retry for transient SSH drops)
...
for config in $FLEET_CONFIGS; do
    ...
    # Copy container files (with retry for transient SSH drops)
    for _attempt in 1 2 3; do
        ...
        echo "    Container files SCP failed (attempt $_attempt), retrying in 5s..."
        sleep 5
    done
    ...
    for _attempt in 1 2 3 4 5; do
        ...
        echo "    hostname SSH failed (attempt $_attempt), retrying in 10s..."
        sleep 10
    done

    # Configure systemd unit with override.conf (idempotent — safe to retry)
    for _attempt in 1 2 3 4 5; do
...
ENVEOF
        ...
        echo "    configure SSH failed (attempt $_attempt), retrying in 10s..."
        sleep 10
    done

    # Build container image (with retry for transient SSH drops)
    for _attempt in 1 2 3; do
        if ssh $SSH_OPTS root@"$pub_ip" "cd /opt/platform/container && docker build -t platform/app-container ." 2>/dev/null; then
            break
        fi
        echo "    docker build SSH failed (attempt $_attempt), retrying in 5s..."
        sleep 5
    done

    # Verify DRBD module (retry — SSH may drop briefly after heavy docker build)
    for _attempt in 1 2 3; do
        if ssh $SSH_OPTS root@"$pub_ip" "modprobe drbd" 2>/dev/null; then
            break
        fi
        echo "    DRBD modprobe SSH failed (attempt $_attempt), retrying in 5s..."
        sleep 5
    done
    ...
    for _attempt in 1 2 3; do
        ...
        echo "    machine-agent start SSH failed (attempt $_attempt), retrying in 5s..."
        sleep 5
    done
