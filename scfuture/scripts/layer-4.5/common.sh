#!/bin/bash
# common.sh — shared functions for Layer 4.5 scripts
...
save_ips() {
    ...
    cat > "$IP_FILE" << EOF
COORD_PUB_IP=$(hcloud server ip l45-coordinator)
COORD_PRIV_IP=$(hcloud server describe l45-coordinator -o json | jq -r '.private_net[0].ip')
FLEET1_PUB_IP=$(hcloud server ip l45-fleet-1)
FLEET1_PRIV_IP=$(hcloud server describe l45-fleet-1 -o json | jq -r '.private_net[0].ip')
FLEET2_PUB_IP=$(hcloud server ip l45-fleet-2)
FLEET2_PRIV_IP=$(hcloud server describe l45-fleet-2 -o json | jq -r '.private_net[0].ip')
FLEET3_PUB_IP=$(hcloud server ip l45-fleet-3)
FLEET3_PRIV_IP=$(hcloud server describe l45-fleet-3 -o json | jq -r '.private_net[0].ip')
EOF
