...
ssh $SSH_OPTS root@"$COORD_PUB_IP" "systemctl start coordinator"
...
for config in $FLEET_CONFIGS; do
    ...
    # Configure systemd unit with NODE_ID, NODE_ADDRESS, and COORDINATOR_URL
    ssh $SSH_OPTS root@"$pub_ip" "
        sed -i 's/PLACEHOLDER_NODE_ID/$node_id/' /etc/systemd/system/machine-agent.service
        # Add NODE_ADDRESS and COORDINATOR_URL to the service
    ...
    # Start machine agent
    ssh $SSH_OPTS root@"$pub_ip" "systemctl start machine-agent"
