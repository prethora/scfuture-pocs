...
func (a *Agent) ContainerStart(userID string) (*ContainerStartResponse, error) {
	...
	return &ContainerStartResponse{ContainerName: containerName}, nil
}
...
func (a *Agent) ContainerStatus(userID string) (*ContainerStatusResponse, error) {
	if err := validateUserID(userID); err != nil {
		return nil, err
	}
	...
	if !a.containerExists(containerName) {
		return &ContainerStatusResponse{Exists: false, Running: false}, nil
	}
